...
|---|---|---|
| `python -m pytest tests/ -q` | 56-test suite: API contracts, dashboards, feedback (incl. alert-ownership guards), auth shadow mode, input robustness, malformed-request 400s, session-level toxicity-streak alert, the parent-only data export (secrets excluded, scope = deletion scope), and ML-unit tests (shared audio extractor, noisy-OR fusion, Hinglish lexicon, deleted-account token check, threshold-tuner math) — isolated throwaway DB | `56 passed` |
...
```
29 tests across the two apps pin the pure decision logic: the offline chat queue's
