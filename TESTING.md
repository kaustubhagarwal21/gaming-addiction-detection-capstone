...
| `python ml/eval_asr_fairness.py --transcribe` then `--report` | ASR accent-fairness audit on Svarah (AI4Bharat, 6,656 clips / 117 speakers, CC BY 4.0; HF-login-gated download — recipe in the script docstring): WER **and** benign-speech toxicity-alert FPR through the exact served STT→scorer chain, stratified by language family / primary language / gender → `docs/asr_fairness.json`. Transcription is resumable (`SHARD`/`N_SHARDS` split the clips across workers) |
