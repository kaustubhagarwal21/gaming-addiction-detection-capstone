...
|---|---|---|
| `python -m pytest tests/ -q` | 55-test suite: API contracts, dashboards, feedback (incl. alert-ownership guards), auth shadow mode, input robustness, session-level toxicity-streak alert, the parent-only data export (secrets excluded, scope = deletion scope), and ML-unit tests (shared audio extractor, noisy-OR fusion, Hinglish lexicon, deleted-account token check, threshold-tuner math) — isolated throwaway DB | `55 passed` |
...
```
22 tests across the two apps pin the pure decision logic: the offline chat queue's
delivered-or-deliberately-dropped semantics (`ChatQueueLogic`), the WAV container
header the voice channel depends on (`WavUtil`), and the parent-alert triage rules
incl. regressions for the sibling high-water-mark and risk-flap bugs (`AlertTriage`).
