...
Mechanical loanword code-mixing of CONDA & \textbf{rejected} & measured whack-a-mole: clean-only conversion collapsed an abusive transcript ($0.73 \rightarrow 0.45$); label-neutral conversion pushed held-out precision below the $0.95$ bar (\texttt{ml/make\_hindi\_gaming\_codemixed.py} kept as the negative result) \\
...
scripts, + the clean-Hindi counterweight; held-out Brier $0.066 \rightarrow 0.063$), fused with the keyword lexicon. The held-out set is
...
\begin{itemize}[leftmargin=1.4em]
  ...
  \item \textbf{Code-mixed text model and Hindi speech recognition.} The lexicon
        ...
        double-count toward the streak alert. The mode ships behind a
        ...
        battery with two models resident), the deployed default behaviour is
        byte-identical to v2.3.11.
\end{itemize}
