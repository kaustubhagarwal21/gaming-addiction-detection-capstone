...
($\Delta\rho = +0.167$, $95\%$ CI $[-0.002, +0.341]$), and survives partialling
that baseline out
...
\begin{enumerate}[leftmargin=1.4em]
  ...
  \item An evaluation methodology with teeth: model-family bake-offs for all three
        ...
        8.3-point leakage inflation and \emph{reversed} a model-selection
...
CONDA (real Dota~2 chat) & adopted & the in-domain anchor: its removal collapses PR-AUC $0.825 \rightarrow 0.511$ \\
...
reads $0.657$ under random splits and $0.574$ speaker-independent, an $8.3$-point
...
on this same held-out CONDA split --- \textbf{12 points below the
classical pipeline} ($0.825$ under the ablation protocol; $0.821$ as served) --- and at a $\geq$$0.95$-precision operating point
...
General realistic stream @0.50 & 0.235 & 0.956 & 3.5\% toxic base rate \\
...
is $0.82$ in-domain at precision $0.63$), so repeated toxicity in a session
...
$8.3$-point gap \emph{is the quantified speaker leakage} --- the model partly
...
\begin{itemize}[leftmargin=1.4em]
  ...
  \item A layered test program, all passing: a 181-test API suite (isolated database;
  ...
  \item Robustness under load on free-tier hosting: pooled, probe-validated connections
        ...
        landed --- p50 66\,ms, p95 684\,ms --- via the permanent
...
$97.4\%$ of resamples (a one-sided bootstrap $p \approx 0.026$) --- though the
two-sided interval now grazes zero ($95\%$ CI $[-0.002, +0.341]$; at the
pre-batch $n{=}86$ it excluded zero), so superiority over the baseline is
suggestive, not established, and we say so. (The construct-validity correlation
of Result~1 was the pre-designated primary endpoint --- stated in the analysis
script's documentation before data collection; the remaining results are
secondary.) The partial correlation establishes \emph{non-redundancy}:
partialling self-reported hours out of the model's correlation barely moves it
($\rho = 0.303$, $95\%$ CI $[0.110, 0.476]$, excluding zero), so the score is
not screen time re-expressed in another unit --- though because hours is a
single, noisily measured item while the score aggregates seven, part of the
model's edge may be differential reliability rather than construct coverage,
and partialling an unreliable covariate under-controls for true screen
time~\cite{westfall2016}. Non-redundancy plus the pattern-versus-volume
contrast of Result~3 --- not the paired delta --- is what carries the design
premise.
...
which does not exclude zero). The contrast is tested formally, not eyeballed: a
paired bootstrap on the composite difference gives $+0.199$ ($95\%$ CI
$[+0.019, +0.380]$, excluding zero; pattern ahead in $98.5\%$ of resamples).
Individual per-feature rankings carry standard errors of ${\approx}0.10$ at
this $n$ and are descriptive only. Two features carry almost nothing:
...
ordered by magnitude (per-feature SEs ${\approx}0.10$; individual rankings are
descriptive). The formal test is the paired composite contrast: pattern $-$
volume $= +0.199$, $95\%$ CI $[+0.019, +0.380]$. Composites are sign-corrected
$z$-score sums with bootstrap CIs.}
...
member was notified in writing: an e-mail of 14~August~2026 from the first
author's official institutional account, copied to the other team members, disclosing
the design (adults-only, anonymous, consent-gated, voluntary) and explicitly
requesting direction should the department require additional approval. The
notification was sent while the form was open --- after the main collection
wave (7--9~August) but before close-out and before any analysis of the final
sample. That is faculty notification and documented good-faith process; it is
\emph{not} an ethics-committee determination, and we do not describe it as one. Any cohort
...
\bibitem{westfall2016} J. Westfall and T. Yarkoni, ``Statistically controlling for
confounding constructs is harder than you think,'' \emph{PLoS ONE}, vol.~11, no.~3,
e0152719, 2016.
\end{thebibliography}
