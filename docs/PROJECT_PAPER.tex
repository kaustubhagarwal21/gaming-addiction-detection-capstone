...
is grounded in \emph{ten real, open datasets}: the chat model reaches $0.97$
...
$\geq$$0.95$ precision on each script); the model is evaluated on CONDA's \emph{held-out} validation split it
...
\begin{itemize}[leftmargin=1.4em]
  ...
  \item \textbf{Chat:} TF-IDF over a \emph{union} of word 1--2 grams and
        ...
        held-out Brier $0.068 \rightarrow 0.066$). The character channel was \emph{originally rejected} --- trained on
...
\S\ref{sec:chatresults}. Capture is gated to
...
scripts; held-out Brier $0.068 \rightarrow 0.066$), fused with the keyword lexicon. The held-out set is
...
\midrule
Balanced held-out & 0.886 & 0.958 & optimistic view; PR-AUC 0.937 \\
General realistic stream @0.50 & 0.241 & 0.958 & 3.5\% toxic base rate \\
General realistic stream @0.95 (alert) & 0.458 & 0.279 & out-of-domain view \\
\bottomrule
\end{tabular}
\end{center}
...
\midrule
@0.95 (deployed alert threshold) & \textbf{0.971} & 0.285 & 0.440 \\
@0.90 (recall-preferring option, env-selectable) & 0.927 & 0.590 & 0.721 \\
@0.85 (near best F1; selectable via env) & 0.851 & 0.685 & 0.759 \\
@0.97 (high-severity marker) & 1.000 & 0.025 & 0.048 \\
\bottomrule
\end{tabular}
\end{center}
The promised secondary views on this in-domain set: ROC-AUC $0.921$ and MCC $0.477$
...
rows romanised). Two measured adoptions closed both registers. First the
...
(ITRANS-based; the fetch script reproduces it deterministically). On the
...
alert threshold: \textbf{Devanagari PR-AUC $0.873$, precision $0.958$ / recall
$0.320$; romanised PR-AUC $0.872$, precision $0.973$ / recall $0.217$} ---
...
(CONDA PR-AUC $0.827$ vs.\ $0.833$, inside the bootstrap CI). Capture-path
...
training data and HASOC 2019 cannot be excluded --- but $0.837$ on the
\emph{romanised} view, \emph{below} the deployed pipeline's $0.872$: on the
...
(audit and Hindi paragraphs above) hold at $\mathbf{0.827}$ served,
statistically unchanged through all three. Each revision \emph{re-derived the
...
$0.971$ at recall $0.285$ on English in-domain chat --- deliberately above the
$\sim$$0.95$-precision point so that \emph{every} register clears $0.95$
precision at one shared threshold (the same curve's $0.90$ point reads
$0.927/0.590$, available to a guardian who prefers recall ---
\texttt{CHAT\_ALERT\_T} is env-tunable), and the high-severity marker ($0.97$)
is right every time it fires (precision $1.00$ at recall $0.025$). The
...
\midrule
Full recipe (deployed)            & 0.830 [0.813, 0.845] & 0.959 \\
$-$ keyword-lexicon fusion        & 0.821 [0.804, 0.837] & 0.958 \\
$-$ calibration                   & 0.841 [0.825, 0.856] & 0.969 \\
$-$ char\_wb n-grams              & 0.802 [0.784, 0.819] & 0.956 \\
$-$ Davidson corpus               & 0.806 [0.789, 0.823] & 0.931 \\
$-$ HASOC Hindi corpus (both scripts) & 0.831 [0.813, 0.846] & 0.956 \\
$-$ CONDA corpus (domain data)    & 0.534 [0.508, 0.557] & 0.969 \\
Original (general-only, word-only) & 0.516 [0.490, 0.539] & 0.889 \\
\bottomrule
\end{tabular}
\end{center}
Domain data dominates everything else combined ($+0.30$ PR-AUC) --- removing CONDA
collapses the model below even the original recipe's curve shape. Character n-grams
($+0.028$) and Davidson ($+0.024$) are
each significant. The HASOC Hindi row is deliberately read differently: its CONDA
delta is $\approx 0$ \emph{by design} --- a Hindi corpus cannot move an
...
from near-zero), which is
why corpus adoptions are judged on capability, not on a single aggregate score.
Two nuances: the keyword fusion buys only $+0.009$ PR-AUC --- and a telling
...
dual-script HASOC Hindi corpus) is $0.068 \rightarrow 0.066$.
...
\begin{itemize}[leftmargin=1.4em]
  ...
  \item \textbf{Code-mixed text model and Hindi speech recognition.} The lexicon
        ...
        abusive utterances $0.73$--$0.95$ vs.\ clean controls $0.17$--$0.61$ ---
        short spoken lines land below the per-message alert but all clear the
        $0.6$ streak bar, so the session-level path would carry spoken-Hindi
        detection. The test also surfaced the calibration item to fix first: the
        Hindi training rows' $53\%$ offensive base rate leaves clean Devanagari
        slightly prior-elevated (one clean control read $0.61$, at the streak
        bar), so the Hindi corpus needs clean-text balancing before a rollout.
        (Synthetic studio speech; field conditions remain the on-device
        question.) A third, smaller
        item closes the one remaining capture blind spot (\S5.2): a transliteration
        layer on the bundled QWERTY keyboard would route Devanagari through the IME
        path everywhere --- including canvas games, where a third-party Hindi
        keyboard currently bypasses capture entirely.
\end{itemize}
