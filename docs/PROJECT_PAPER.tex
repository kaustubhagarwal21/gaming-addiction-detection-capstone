...
\begin{itemize}[leftmargin=1.4em]
  ...
  \item A layered test program, all passing: a 35-test API suite (isolated database),
        a \textbf{72-check functional sweep run in production auth mode} with real
...
Engineering & Dual-dialect DB with pooled validated Postgres connections, per-client rate limiting, hot-path caching/throttling + audio-concurrency cap + bounded throttle caches, signed APKs, layered tests (35 API + 72 production-mode + 25 live-cloud) + concurrency smoke, real-device validated, lint-clean, deployed + durable DB \\
\bottomrule
\end{tabularx}
\end{center}
