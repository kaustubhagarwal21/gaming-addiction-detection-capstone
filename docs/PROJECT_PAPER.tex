...
\begin{itemize}[leftmargin=1.4em]
  ...
  \item A layered test program, all passing: a 35-test API suite (isolated database),
        a \textbf{73-check functional sweep run in production auth mode} with real
  ...
  \item Robustness under load on free-tier hosting: pooled, probe-validated connections
        ...
        (concurrent game-detection cache; a snapshot-and-merge offline chat queue that
        cannot lose a line captured mid-flush), and background push declares a default
  ...
  \item Graceful degradation when a modality is absent; an idempotent session-end (a
        ...
        existing result instead of double-counting); a self-healing stale-session guard
        that also \emph{scores} the sessions it closes (from the snapshots captured
        live, so a lost end-event still appears in the risk history rather than
        vanishing); and an on-device offline retry queue for chat captured without
        connectivity (kept on a server error, so nothing is silently dropped).
\end{itemize}
...
Engineering & Dual-dialect DB with pooled validated Postgres connections, per-client rate limiting, hot-path caching/throttling + audio-concurrency cap + bounded throttle caches, thread-safe device-side shared state (concurrent cache, snapshot-merge offline queue), signed APKs, layered tests (35 API + 73 production-mode + 25 live-cloud) + concurrency smoke, real-device validated, lint-clean, deployed + durable DB \\
\bottomrule
\end{tabularx}
\end{center}
