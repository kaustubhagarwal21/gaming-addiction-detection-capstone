...
speech clips across $154$ speakers in three languages and evaluated
...
\begin{itemize}[leftmargin=1.4em]
  ...
  \item A layered test program, all passing: a 68-test API suite (isolated database;
...
\begin{itemize}[leftmargin=1.4em]
  ...
  \item \textbf{Tamper/permission monitoring caught two real events.} On one device
        ...
        participant's right.
  \item \textbf{The fusion priors were stress-tested against the pilot's own
        predictions --- and ranked.} The 40/30/30 weights, genre multiplier and
        0.33/0.67 thresholds are stated priors, so we measured how much each one
        actually drives outcomes (\texttt{ml/analyze\_fusion\_sensitivity.py}; the
        re-implementation first reproduces every served score to within $10^{-4}$).
        Across 1{,}961 pilot predictions: perturbing the \emph{ensemble weights}
        anywhere within $\pm 0.15$ of the prior changes the served band for at most
        $0.3\%$ of predictions (median $0.0\%$) --- the weights are demonstrably
        immaterial in practice. The \emph{genre multiplier} is the opposite: removing
        it flips $36\%$ of bands and doubling its effect flips $51\%$ (the pilot's
        casual-genre $0.70$ discount is load-bearing), and $\pm 0.05$ on the
        \emph{thresholds} flips up to $20\%$. This ranks the calibration agenda by
        evidence: genre weighting and band thresholds need real-label calibration
        first; the ensemble weights, whose ``unvalidated'' status reads as the
        headline concern, measurably are not one. A prevalence-anchoring method for
        the top-band threshold (match the share of children in the top band to the
        IGDS9-SF disordered-range rate) is implemented and population-gated
        (\texttt{ml/calibrate\_thresholds\_prevalence.py}) --- it activates when the
        cohort, or a locally collected IGDS9-SF survey, provides a population.
...
\begin{itemize}[leftmargin=1.4em]
  \item \textbf{Feedback-driven retraining, stage two.} Threshold tuning from parent
        ...
        (\Sef{sec:pilot}) already \emph{prioritises} this agenda: genre weighting and band
        thresholds measurably drive outcomes and go first; the ensemble weights
        measurably do not ($\leq 0.3\%$ band impact) and can wait. Needs a pilot at
  ...
  \item \textbf{Voice domain-shift correction (instrumented, evaluation in progress).}
        ...
        have served on live pilot audio. The honest limit stands: (i)--(iii) suppress
