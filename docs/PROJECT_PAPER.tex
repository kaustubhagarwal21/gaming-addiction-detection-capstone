...
parent-visible session --- durability and de-duplication mirror the offline chat queue.
...
captured offline appears as soon as the deferred uploads reach the server. The one
residual gap is a session played \emph{entirely} offline that also ends before
connectivity returns (no online continuation to anchor the back-fill); this is rarer
than the brief-outage-at-start case the buffer now covers, and closing it fully is
...
\begin{itemize}[leftmargin=1.4em]
  ...
  \item A layered test program, all passing: a 70-test API suite (isolated database;
        ...
        Android Lint with zero errors, and 46 JVM unit tests (run in CI) pin the
