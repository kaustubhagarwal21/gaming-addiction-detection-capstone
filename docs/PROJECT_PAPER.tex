...
Reliability/anti-tamper & Heartbeat watchdog (uninstall/kill/offline alert, child-local quiet hours), optional Device Admin (uninstall block + instant disable alert), parent-PIN lock on logout/settings, logout alert, self-restart + boot resume, multi-guardian FCM push (lazy-init) \\
...
\begin{itemize}[leftmargin=1.4em]
  \item \textbf{Real-audio voice model + VAD.} Retrain the voice model on free, real
        labelled speech (RAVDESS~\cite{ravdess2018}) and add voice-activity detection
        (\texttt{webrtcvad}) so silence and game audio are not classified as emotion. Voice
