...
\begin{itemize}[leftmargin=1.4em]
  \item \textbf{Feedback-driven retraining, stage two.} Threshold tuning from parent
        ...
        population prevalence anchor meanwhile). Two open research datasets are
        the named interim resources for de-synthesising the behaviour
        \emph{features} before child data exists at scale: StudentLife (Dartmouth;
        ten weeks of real phone-usage telemetry from 48 students, incl.\ lock/unlock
        and app-usage traces) and LifeSnaps (71 participants, four months,
        multimodal) --- both are adult/student cohorts, so they can ground
        feature \emph{distributions} and session-pattern realism but not the
        child-population labels, the same distribution-vs-label boundary drawn
        in \S4.2.
