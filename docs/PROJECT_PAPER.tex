...
core claim is that multimodal behaviour patterns beat screen-time counting, so the
...
two-band errors on the 5{,}000-row test set, a failure mode \emph{every} multimodal
...
\begin{itemize}[leftmargin=1.4em]
  ...
  \item \textbf{Channel contribution to the fused decision, measured on real
        ...
        (union of independent evidence) would have flipped $46.3\%$ of session
        bands and max-pooling $33.6\%$ --- and in \emph{every single case the flip
