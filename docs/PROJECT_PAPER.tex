...
grounded in \emph{nine real, open datasets}: the chat model is trained on real
...
drift monitor for the pilot phase --- and a consented live family pilot has since
exercised every capture channel and alert type in production (55 auto-detected
sessions and 1{,}660 served predictions in its first 12 days, including the full
spoken-abuse chain firing unprompted) --- and provides a parent-to-child nudge channel so
...
the open session at its last recorded activity, so its duration stays realistic. Screen
...
captured offline appears as soon as the deferred uploads reach the server. A natural
extension identified for future work is an on-device session buffer that records
start/end times locally and back-fills them when connectivity returns, closing the
``session started while offline'' gap.
...
path), so no training row can leak into it --- an earlier draft rebuilt this split
from the general corpus alone, which admitted trained CONDA/Davidson rows and
understated the realistic base rate.
\begin{center}
\begin{tabular}{lccc}
\toprule
...
below behaviour's $0.40$). The earlier measured
...
\S\ref{sec:modelselection}), honest speaker-independent accuracy $0.574$ \\
\addlinespace
...
\begin{itemize}[leftmargin=1.4em]
  ...
  \item A layered test program, all passing: a 65-test API suite (isolated database;
        ...
        Android Lint with zero errors, and 38 JVM unit tests (run in CI) pin the
...
\textbf{0 rows retain audio}; label skew: 77\% frustrated (see below) \\
...
\begin{itemize}[leftmargin=1.4em]
  ...
  \item \textbf{Availability-weighted fusion proved necessary, not hypothetical.}
        ...
        for.
  \item \textbf{Tamper/permission monitoring caught a real event:} the custom keyboard
        was silently disabled on the pilot device mid-pilot (an OS default-IME reset);
        the heartbeat's capture-permission flags flipped, the parent dashboard showed
        ``monitoring degraded'', and a one-shot alert fired --- the designed failure
        visibility, observed in the wild.
  ...
  \item \textbf{Real parent feedback validated the alert threshold.} The Beta-posterior
        tuner (\S5.6), run on the pilot's 17 verdicts, put the probability of an
        excessive false-alarm rate at $0.001$ and recommended \emph{zero change} to the
        deployed $0.90$ chat-alert threshold --- the first real-label corroboration of
        an operating point previously chosen from held-out curves alone.
  \item \textbf{The drift monitor's first scheduled run taught an operations lesson:}
        it flagged PSI up to $5.97$ --- correctly detecting not model drift but the
        \emph{population handover} from seeded demo data to real pilot traffic. The
        monitor now clamps its windows to a data-regime epoch (the pilot start) so
        cross-regime comparisons cannot masquerade as drift.
