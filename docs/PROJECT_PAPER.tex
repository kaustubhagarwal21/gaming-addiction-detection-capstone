...
threaded worker is sized so the whole ML stack stays within memory.
...
authenticated session cannot rewrite or erase its safeguards. Because Android gives an app no callback at its own uninstall,
...
Reliability/anti-tamper & Heartbeat watchdog (uninstall/kill/offline alert, child-local quiet hours), optional Device Admin (uninstall block + instant disable alert), battery-optimisation exemption, double-start-safe services, parent-PIN lock on logout/settings (server-verified, constant-time), parent-role-only PIN change + data deletion, logout alert, self-restart + boot resume, multi-guardian FCM push (lazy-init) \\
Privacy/Security & Token auth + role checks, peppered PIN hashes, HTTPS-only, encrypted token store, consent (versioned), raw-audio deletion, retention purge, 16\,MB body cap \\
Engineering & Dual-dialect DB with pooled validated Postgres connections, per-client rate limiting, hot-path caching/throttling, signed APKs, 35 tests + concurrency smoke, lint-clean, deployed + durable DB \\
\bottomrule
\end{tabularx}
\end{center}
