...
is grounded in \emph{ten real, open datasets}: the chat model reaches $0.95$
...
Devanagari path now \emph{trained and held-out-validated} on the HASOC 2019
Hindi corpus (precision $0.95$, recall $0.44$ at the alert threshold, from zero
model coverage before adoption); the voice model
...
\begin{enumerate}[leftmargin=1.4em]
  ...
  \item An \textbf{India-ready abuse channel} for code-mixed gaming chat: a curated
        ...
        calibrated English toxicity model via noisy-OR --- ablation-measured to lift
        recall at the alert threshold from $0.332$ to $0.385$ (precision
        $0.97 \rightarrow 0.95$) --- a \emph{trained} Devanagari path (HASOC 2019
        Hindi corpus: held-out alert precision $0.95$ where the pre-adoption model
        caught nothing, \S6.2) --- and an
  ...
  \item Grounding in ten real, open datasets (two behaviour-grounding surveys, four
...
breaks, re-logins) have no public source. The chat model is trained on \emph{four
...
labeled Devanagari/Hindi social-media posts; an $80\%$ split is trained on, the
remaining $20\%$ is held out for the Hindi evaluation of
\S\ref{sec:chatresults} --- adopted 2026-08-04 after a measured trial showed the
in-domain CONDA metrics hold while Devanagari alert capability goes from zero to
precision $0.95$); the model is evaluated on CONDA's \emph{held-out} validation split it
...
HASOC 2019 Hindi~\cite{hasoc2019} & adopted & real Devanagari/Hindi abuse; measured trial: CONDA holds in-CI, Hindi alert capability $0 \rightarrow 0.95$ precision \\
...
(\texttt{unitaryai/detoxify}, $\sim$110M parameters) reads PR-AUC $\mathbf{0.709}$
on this same held-out CONDA split --- \textbf{12 points below the deployed
classical pipeline} ($0.833$) --- and at a $\geq$$0.95$-precision operating point
...
the word~$\cup$~char\_wb model trained on four real corpora (general toxicity +
CONDA train split + Davidson offensive-language tweets + HASOC 2019 Hindi; held-out
Brier $0.066 \rightarrow 0.064$), fused with the keyword lexicon. The held-out set is
...
\midrule
Balanced held-out & 0.889 & 0.956 & optimistic view; PR-AUC 0.937 \\
General realistic stream @0.50 & 0.231 & 0.956 & 3.5\% toxic base rate \\
General realistic stream @0.95 (alert) & 0.426 & 0.436 & out-of-domain view \\
\bottomrule
\end{tabular}
\end{center}
...
$0.95$. The subsequently adopted HASOC Hindi corpus (\S4.2) re-measured these
figures once more (its Devanagari rows make the balanced holdout slightly
harder --- accuracy $0.918$, Brier $0.064$ --- the price of a whole new language
in the corpus); the tables in this section report the final deployed model.
...
\midrule
@0.95 (deployed alert threshold) & \textbf{0.954} & 0.423 & 0.586 \\
@0.90 (recall-preferring option, env-selectable) & 0.924 & 0.577 & 0.711 \\
@0.85 (near best F1; selectable via env) & 0.801 & 0.710 & 0.753 \\
@0.97 (high-severity marker) & 1.000 & 0.027 & 0.053 \\
\bottomrule
\end{tabular}
\end{center}
The promised secondary views on this in-domain set: ROC-AUC $0.920$ and MCC $0.584$
at the alert threshold ($0.680$ at the $0.90$ point; per-configuration values for
every ablation row are in the released \texttt{ablation\_results.json} artifact).

\paragraph{Held-out Hindi/Devanagari evaluation (new capability, measured).}
...
Hindi set --- the Hinglish keyword lexicon covers romanised abuse, but no
trained model component read Devanagari). On the $933$-row HASOC 2019 Hindi
held-out split ($20\%$, stratified, never trained on; $52.9\%$ offensive base
rate), the deployed model now reads \textbf{PR-AUC $0.875$, precision $0.948$ /
recall $0.439$ at the alert threshold} --- the same precision-first profile as
the English in-domain operating point, on a language the model previously could
not score at all. The in-domain CONDA metrics held within the bootstrap CI
through the adoption (PR-AUC $0.826$ vs.\ $0.833$) with alert-point recall
\emph{up} nine points --- the measured trial that justified adoption
(\texttt{ml/eval\_chat\_hindi.py} reproduces this evaluation).
...
$0.834$; the post-pilot de-duplicated retrain (audit above) holds at
$\mathbf{0.833}$ served, statistically unchanged. Each revision \emph{re-derived the
alert threshold from its own held-out curve} --- calibration shifts where
probabilities land, so a threshold never survives a model change unexamined. On the
retrained model's calibrated scale the $\sim$$0.95$-precision operating point sits at
threshold $0.95$: the deployed alert reads precision $0.962$ at recall $0.334$
(the same curve's $0.90$ point reads $0.922/0.606$, available to a guardian who
prefers recall --- \texttt{CHAT\_ALERT\_T} is env-tunable), and the high-severity
marker ($0.97$) is right $99\%$ of the time. The pre-dedupe deployment's point read
$0.950/0.491$ on its own scale, itself strictly dominating its predecessors
($0.924/0.445$ before Davidson, $0.891/0.266$ for the original general-corpus
model). Per-message recall deliberately traded away here is recovered at session
level by the streak alert below.
...
is $0.87$ in-domain at precision $0.53$), so repeated toxicity in a session
...
(2026-08-04): the audEERING wav2vec2 \emph{dimensional} emotion model (trained on
...
(Future Work names the deployable path). That weight is \emph{nominal}: availability reweighting
...
\midrule
Full recipe (deployed)            & 0.833 [0.816, 0.849] & 0.954 \\
$-$ keyword-lexicon fusion        & 0.827 [0.809, 0.843] & 0.965 \\
$-$ calibration                   & 0.844 [0.827, 0.859] & 0.964 \\
$-$ char\_wb n-grams              & 0.799 [0.781, 0.817] & 0.957 \\
$-$ Davidson corpus               & 0.796 [0.778, 0.814] & 0.944 \\
$-$ HASOC Hindi corpus            & 0.831 [0.813, 0.846] & 0.956 \\
$-$ CONDA corpus (domain data)    & 0.523 [0.498, 0.547] & 0.964 \\
...
($+0.034$) and Davidson ($+0.037$) are
each significant. The HASOC Hindi row is deliberately read differently: its CONDA
delta is $\approx 0$ \emph{by design} --- a Devanagari corpus cannot move an
English-chat metric --- and its measured contribution lives entirely in the
held-out Hindi evaluation above ($0 \rightarrow 0.95$ alert precision), which is
why corpus adoptions are judged on capability, not on a single aggregate score.
Two nuances: the keyword fusion buys only $+0.006$ PR-AUC
--- it is retained primarily for \emph{coverage} (romanised-Hinglish abuse the
TF-IDF vocabulary cannot see) and interpretability, not ranking. And
\textbf{calibration costs $0.011$ PR-AUC}: isotonic regression creates probability
...
HASOC Hindi corpus) is $0.066 \rightarrow 0.064$.
...
\label{sec:pilot}
A consented family pilot ran from 6~July~2026 to early August 2026 ($\sim$4 weeks):
release APKs sideloaded onto family
...
unit-tested, and an automated independent static audit (MobSF v4.5.1, August
...
\begin{itemize}[leftmargin=1.4em]
  ...
  \item \textbf{Voice domain-shift correction (instrumented, evaluation in progress).}
        ...
        audEERING wav2vec2 dimensional model (natural-speech-trained;
        ...
        required, not a drop-in swap), its 72K-parameter Wav2Small distillation
        (small enough for the current serving budget), and the EmoReact corpus
