...
\begin{itemize}[leftmargin=1.4em]
  ...
  \item A layered test program, all passing: a 35-test API suite (isolated database),
        a \textbf{81-check functional sweep run in production auth mode} with real
...
Privacy/Security & Token auth + \emph{role} checks on every parent-only endpoint, peppered PIN hashes (validated, family-wide PIN change), HTTPS-only, encrypted token store, consent (versioned), raw-audio deletion, retention purge, 16\,MB body cap, family code always recoverable in PIN-gated Settings \\
Engineering & Dual-dialect DB with pooled validated Postgres connections, per-client rate limiting, hot-path caching/throttling + audio-concurrency cap + bounded throttle caches, thread-safe device-side shared state (concurrent cache, crash-safe confirm-then-delete offline queue), signed APKs, layered tests (35 API + 81 production-mode + 25 live-cloud) + concurrency smoke, real-device validated, lint-clean, deployed + durable DB \\
\bottomrule
\end{tabularx}
\end{center}
...
\textbf{Tier 1 --- Immediate, high-value extensions (engineering only).}
The near-term work genuinely worth doing with the existing codebase and free tools ---
adding visible credibility and implementable directly --- is to evaluate (and, where useful,
retrain) two of the three models on \emph{real, public} labelled data, with no data
collection required:
\begin{itemize}[leftmargin=1.4em]
  \item \textbf{Real-audio voice model + VAD.} Retrain the voice model on free, real
        labelled speech (RAVDESS~\cite{ravdess2018}, CREMA-D) and add full voice-activity
        detection (\texttt{webrtcvad}). The served pipeline already applies an RMS silence
        floor so silent segments are not classified as emotion; real VAD additionally
        rejects \emph{game audio and background speech}, which an energy floor cannot. Voice
  ...
  \item \textbf{Chat model on a real toxicity benchmark.} Evaluate the chat-toxicity model
        on a public dataset --- a general benchmark or, better, an in-game toxicity corpus
        --- and report precision/recall/F1/AUC beside the synthetic figures. No recruitment
        is needed; the model and the honest-metrics infrastructure already exist.
\end{itemize}
