...
adult survey ($n{=}87$), the score the system would have served correlates with
the clinically-validated IGDS9-SF instrument at Spearman $\rho = 0.352$ ($95\%$
CI $[0.158, 0.521]$) and \emph{beats the self-reported screen-time baseline it
is meant to replace} (paired $\Delta\rho = +0.195$, $95\%$ CI
$[+0.026, +0.372]$), with the signal concentrated in the behavioural-pattern
features rather than the volume ones --- the project's core premise, confirmed
...
\begin{enumerate}[leftmargin=1.4em]
  ...
  \item \textbf{External construct validation against a clinical instrument}
        (\S\ref{sec:survey}). An anonymous adult survey ($n{=}87$) paired the
        IGDS9-SF~\cite{igds9sf} with gaming-pattern questions scored through the
        \emph{deployed} serving path. The risk score tracks IGDS9-SF severity
        ($\rho = 0.352$, $95\%$ CI $[0.158, 0.521]$); it beats the self-reported
        screen-time baseline on a paired bootstrap ($\Delta\rho = +0.195$, $95\%$
        CI $[+0.026, +0.372]$) and survives partialling that baseline out
        ($\rho = 0.349$); and the signal is carried by the five \emph{pattern}
        features, whose composite ($\rho = 0.358$) separates cleanly from the
        \emph{volume} composite ($\rho = 0.202$, CI including zero). The same
...
its own: an anonymous adult IGDS9-SF survey ($112$ responses, $87$ usable) run
...
$86$ respondents, the pattern composite reaches $\rho = 0.358$ and the volume
composite only $0.202$ with a CI including zero --- the same ordering, obtained
...
\textbf{Sample.} $112$ raw responses yielded $87$ usable ($77.7\%$): $17$ failed
an eligibility gate (under-18 or non-gamer) and $8$ failed the attention check;
$86$ answered every pattern question and are therefore scoreable. The sample is
young ($76.7\%$ of the scoreable $86$ aged $18$--$24$) and skews non-severe:
IGDS9-SF totals average
$17.6$ (SD $6.1$, median $17$, range $9$--$37$), with $2$ respondents
($2.3\%$) at or above the elevated cutoff ($\geq$$32$) and $1$ ($1.1\%$) in the
disordered range ($\geq$$36$). That distribution is the study's governing
...
IGDS9-SF severity at Spearman $\rho = \mathbf{0.352}$ ($95\%$ bootstrap CI
$[0.158, 0.521]$, $n{=}86$). The interval excludes zero, so the model's output
...
\textbf{Result 2 --- it is not a screen-time proxy, and it beats one.} The
baseline every commercial parental-control tool ships is self-reported hours per
week; on the same $86$ respondents that baseline reaches only $\rho = 0.155$
($95\%$ CI $[-0.057, 0.376]$), spanning zero. (The comparison is deliberately
restricted to respondents the model can score, so both predictors are measured
on identical rows; across all $87$ usable respondents the hours baseline reads
$\rho = 0.138$ --- the same verdict.) A paired bootstrap on the \emph{difference} puts the model
ahead by $\Delta\rho = +0.195$ ($95\%$ CI $[+0.026, +0.372]$; the model wins in
$98.6\%$ of resamples), and partialling self-reported hours out of the model's
correlation barely moves it ($\rho = 0.349$, $95\%$ CI $[0.149, 0.521]$). The
score is therefore carrying severity information that volume does not, rather
than re-expressing volume in another unit --- the project's core design premise,
confirmed externally rather than assumed.
...
and the sign-corrected composites diverge: pattern $\rho = 0.358$ ($95\%$ CI
$[0.160, 0.529]$) against volume $\rho = 0.202$ ($95\%$ CI $[-0.011, 0.395]$,
which does not exclude zero). Two features carry almost nothing:
\texttt{daily\_play\_time\_hours} ($+0.066$) and
\texttt{avg\_session\_duration\_min} ($-0.002$). This is the first
...
\centering
\footnotesize
\caption{Per-feature Spearman correlation against IGDS9-SF total ($n{=}86$),
...
\midrule
\texttt{avg\_break\_between\_sessions\_min} & $-0.305$ & pattern \\
\texttt{rapid\_relogin\_ratio}              & $+0.305$ & pattern \\
\texttt{late\_night\_play\_ratio}           & $+0.286$ & pattern \\
\texttt{binge\_sessions\_per\_week}         & $+0.221$ & pattern \\
\texttt{longest\_play\_streak\_days}        & $+0.216$ & pattern \\
\midrule
\texttt{sessions\_per\_day}                 & $+0.189$ & volume \\
\texttt{weekly\_play\_time\_hours}          & $+0.144$ & volume \\
\texttt{days\_played\_per\_week}            & $+0.144$ & volume \\
\texttt{daily\_play\_time\_hours}           & $+0.066$ & volume \\
\texttt{avg\_session\_duration\_min}        & $-0.002$ & volume \\
\midrule
\textbf{pattern composite} & $\mathbf{+0.358}$ \ [$0.160$, $0.529$] & --- \\
\textbf{volume composite}  & $+0.202$ \ [$-0.011$, $0.395$] & --- \\
\bottomrule
\end{tabular}
\end{table}
...
That self-report tracks IGDS severity at $\rho = 0.315$ ($95\%$ CI
$[0.114, 0.498]$, $n{=}86$) --- the same direction as, and stronger than, the
...
Across the seven genres with $n \geq 4$, mean IGDS totals span $15.0$ (Strategy)
to $19.7$ (Battle Royale) in the ordering the deployed multiplier assumes ---
but a Kruskal-Wallis test finds no significant difference ($H = 5.42$,
$p = 0.491$). The honest reading is \emph{underpowered, not disproven}:
resampling from the observed group distributions gives this test only $36\%$
power at the achieved $n$, reaching $90\%$ only near $n \approx 258$ scoreable
respondents. Because the pilot's sensitivity analysis (\S\ref{sec:pilot}) ranks
...
\texttt{craving\_score} does ($\rho = +0.325$ vs the preoccupation item);
\texttt{gaming\_priority\_score} is weak ($+0.162$) and
\texttt{tolerance\_score}, \texttt{control\_loss\_score} and
\texttt{neglect\_responsibilities\_score} are indistinguishable from zero
($+0.063$, $+0.040$, $+0.073$). We report this because it is evidence against
...
correct one. Renaming them to behaviour-descriptive labels is a concrete
follow-up this study directly motivates.

\textbf{Robustness.} Eight respondents gave the identical answer to all nine
items. Dropping them attenuates the headline but does not overturn it:
$\rho = 0.290$ ($95\%$ CI $[0.080, 0.475]$, $n{=}78$), still excluding zero.
...
and $\approx$$860$ at the rate this sample actually showed.
...
attenuates correlations, so $0.352$ is more plausibly a floor than a ceiling ---
a direction that favours the model, which is exactly why we decline to lean on it.
(iii) \emph{Self-reported behaviour, not telemetry.} Respondents reported their
own patterns in bands; the deployed system measures them. Band midpoints
coarsen the features, and self-report adds recall error --- both attenuating.
...
$T_1{=}0.51$, $T_2{=}0.95$ ($\kappa = 0.197$) versus the deployed
$0.33 / 0.67$ prior. We have not applied it. The upper cut-off is pinned near
the top of the scale precisely because the sample contains almost no
disordered-range respondents, so the fitted $T_2$ encodes the sample's missing
tail rather than a clinical boundary --- and $\kappa = 0.197$ is fair agreement
at best. The tuner is wired to environment variables and reversible in one
...
browser gaming:} dedicated cloud-gaming apps (GeForce NOW, Xbox cloud) are
monitorable --- they are caught when they self-declare the OS game category
and are one tap away in the parent's manual game overrides, and every
...
title, and streamed frames are opaque to the accessibility reader (the IME
path still captures typed chat). Games played \emph{inside a browser},
however, are invisible: the browser is not a game, and force-monitoring it
would count all browsing as play. We accept this evasion path deliberately ---
closing it would require screen capture (MediaProjection) or URL surveillance,
both of which violate the no-raw-media, minimum-collection posture this
section defends; a monitoring boundary honestly disclosed to the parent is
preferable to a privacy regression that closes it.
...
instrument in real respondents ($\rho = 0.352$), does so beyond self-reported
screen time (partial $\rho = 0.349$; a paired bootstrap puts it ahead of the
hours baseline in $98.6\%$ of resamples), and that the signal sits in the
...
the study tested and failed to support ($p = 0.491$, at $36\%$ power); it is
...
does replicate the premise that motivates the channel ($\rho = 0.315$). The
...
assert it: an external IGDS9-SF study (\S\ref{sec:survey}, $n{=}87$) shows the
served risk score tracking a validated clinical instrument
($\rho = 0.352$), outperforming the self-reported screen-time baseline it exists
to replace ($\Delta\rho = +0.195$, CI excluding zero), with the signal residing
...
\begin{itemize}[leftmargin=1.4em]
  \item \textbf{Feedback-driven retraining, stage two.} Threshold tuning from parent
        ...
        genre multiplier ($p = 0.491$ at $36\%$ power), which sharpens rather
        ...
        in $87$). Two open research datasets are
  ...
  \item \textbf{Code-mixed text model and Hindi speech recognition.} The lexicon
        ...
        mode: on noise it outputs silence. The mode ships behind a
        parent-gated toggle, default \emph{off}: until the on-device validation
        checklist runs on real hardware (STT quality on real speech, CPU and
        battery with two models resident --- \texttt{TESTING.md} scripts the
        quantified drill, battery attribution / RAM / per-session data with the
        toggle off vs.\ on, whose deltas gate the beta's promotion), the
        deployed default behaviour is byte-identical to v2.3.11.
\end{itemize}
...
\begin{itemize}[leftmargin=1.4em]
  \item \textbf{Expert / clinical validation (stage two).} The first stage ---
        ...
        $1.1\%$. Second, \textbf{the right population}: adults reporting their
  ...
  \item \textbf{ASR fairness audit across Indian accents.} The on-device STT is
        an Indian-English Vosk model, but India's phonetic diversity spans
        Indo-Aryan and Dravidian accent families, and both the word-error rate
        and --- more consequentially --- the toxicity-alert false-positive rate
        conditioned on accent are unquantified. A transcription error that
        \emph{creates} a slur is an equity failure concentrated on whoever the
        acoustic model serves worst, so an alerting system carrying an ASR front
        end owes its users a demographic audit: WER and per-accent alert-FPR
        measured on a labelled, accent-stratified corpus of Indian adolescent
        speech. No such open corpus exists for gaming speech; assembling one
        (consented, ethics-reviewed) is the actual work, and the audit is a
        precondition to any deployment claim of fairness across regions.
