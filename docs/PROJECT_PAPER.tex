...
\begin{itemize}[leftmargin=1.4em]
  ...
  \item \textbf{Code-mixed text model and Hindi speech recognition.} The lexicon
        ...
        functionally validated end to end on real hardware (Devanagari
        capture, dual-language transcription, transparency screens, session
        lifecycle, PIN gates) and promoted to the current release; the mode
        itself ships behind a parent-gated toggle, default \emph{off}. What
        was \emph{not} run is the quantified resource drill --- battery
        attribution, RAM and per-session data with the toggle off vs.\ on,
        which 	exttt{TESTING.md} scripts with an explicit acceptance gate
        and whose results table is, at the time of writing, still blank. We
        state that rather than infer it: the default-off toggle means a family
        enabling dual-language recognition is opting into a resource cost we
        have not measured, and with the toggle off the deployed behaviour is
        byte-identical to v2.3.11. Running that drill is the first post-defence
        engineering task.
\end{itemize}
