...
favour calibrated, explainable classical models with SHAP attributions over
heavyweight deep models, prioritising on-device privacy, deployability and honesty
of evaluation --- and every model choice is backed by a measured bake-off, every
design choice by an ablation with bootstrap confidence intervals. The pipeline is
grounded in \emph{six real, open datasets}: the chat model is trained on real
in-game and offensive-language corpora, reaches $0.95$ precision at its alert
threshold on held-out real gaming chat, and is built for the Indian context ---
code-mixed Hinglish/Devanagari abuse is detected whether typed or \emph{spoken}
(transcribed on-device, so raw audio never leaves the phone); the voice model is trained on $9{,}817$ real
speech clips across $163$ speakers in three languages and evaluated
speaker-independently; the behaviour generator's play-time distributions and the
dashboard's peer percentiles follow a real survey of $13{,}464$ gamers; and an
$11{,}191$-respondent IGDS9-SF dataset validates both the severity base rate and the
chat channel's core premise. The platform is production-deployed (HTTPS, token
authentication, parental consent, a durable managed database), ships a
parent-feedback loop that converts real verdicts into training labels plus a
drift monitor for the pilot phase, and provides a parent-to-child nudge channel so
the parent can intervene --- not merely observe. Remaining limits are stated
plainly --- behavioural \emph{labels} are synthetic screening priors pending pilot
data --- and the output is framed explicitly as \emph{screening and awareness, not
clinical diagnosis}.
\end{abstract}
...
\begin{enumerate}[leftmargin=1.4em]
  ...
  \item Grounding in six real, open datasets --- each screened against explicit
...
into a single \emph{calibrated, SHAP-explainable} risk band; (iii)~abuse detection built
...
just alerts); and (v)~an honest, realistic-stream evaluation. The novelty lies in the deployed system and the canvas-game
capture technique --- \emph{not} in the individual classical models, which we
deliberately keep lightweight and explainable. Table~\ref{tab:compare} summarises the
comparison.
...
Honest realistic-stream evaluation                      & \capN & \capN & \capP & \capY \\
\bottomrule
\end{tabularx}
\end{table}
...
statistically unchanged ($92.3\%$ on ten features vs.\ $92.4\%$ on twenty) --- so the
change costs nothing and buys honest attributions. The backend sizes its model input
...
real corpora}: a public general-toxicity corpus, the CONDA train
split~\cite{conda2021} (real Dota~2
...
$9{,}817$ clips across $163$ speakers in three languages, all ingested through the
...
A sixth open dataset provides \emph{label-side} grounding: the cross-cultural
...
involvement score significantly higher on IGD severity (\S4.4). It lacks a playtime
...
General toxicity corpus & adopted & chat training base (public, real comments) \\
...
\begin{itemize}[leftmargin=1.4em]
  ...
  \item \textbf{Chat:} TF-IDF over a \emph{union} of word 1--2 grams and
        ...
        held-out Brier $0.066 \rightarrow 0.064$). The character channel was \emph{originally rejected} --- trained on
...
reads $0.657$ under random splits and $0.57$ speaker-independent, a $\approx 9$-point
inflation that also \emph{flipped the model-selection verdict}
(\S\ref{sec:modelselection}).

\textbf{Uncertainty and ablations.} Headline metrics carry bootstrap $95\%$
confidence intervals (1{,}000 test-set resamples), so ``improvements'' inside the
...
\end{tabular}
\end{center}
Every family --- \emph{including the linear baseline} --- lands within $0.7$
percentage points. The reading is important: on synthetic data whose labels come
from the generator's own feature distributions, the \emph{data} is the ceiling, not
the classifier, and chasing the $+0.7$\,pp would be tuning to the generator.
...
score} for every channel (how honest the served probabilities are), and probability
...
``angry'' event. (We note honestly that \texttt{webrtcvad} is a speech-tuned model, not
...
\subsection{Offline behaviour: what is captured, deferred, or lost}
The Child app tolerates intermittent connectivity, but the guarantee differs by data
stream --- a deliberate design choice driven by the stakes and volume of each. The
policy is store-and-forward for the highest-value, lowest-volume evidence, and
best-effort for high-volume telemetry, with the parent always \emph{told} when
monitoring goes dark rather than being shown a silent gap.
\begin{itemize}[leftmargin=1.4em]
  \item \textbf{Typed chat --- fully durable.} A captured line that fails to upload is
        written to a disk-backed, encrypted queue and retried both by the monitor's
        poll loop and by a \texttt{WorkManager} job that fires the moment connectivity
        returns, surviving process death. A line is removed from disk only after the
        server confirms it (a crash mid-upload re-sends on the next flush and the server
        de-dupes), so no typed message is lost offline. The queue is bounded (50 lines;
        entries older than 24\,h are dropped) so it cannot grow without limit.
  \item \textbf{Spoken words --- durable.} Speech-to-text runs entirely on-device
        (Vosk), so transcripts are produced without a network and enter the same durable
        chat queue --- what the child \emph{says} is also captured offline and delivered
        later.
  \item \textbf{Session end --- approximated, not lost.} If an end-event does not reach
        the server, the stale-session watchdog closes the session server-side at its last
        recorded activity, so the duration is estimated rather than dropped.
  \item \textbf{Session start --- lost while offline.} Starting a session requires the
        server to allocate a session id; a game begun with no connectivity is not tracked
        until the network returns, at which point tracking begins from that moment (the
        offline portion is not recovered).
  \item \textbf{Screen events, notification events --- best-effort.} These are
        fire-and-forget and are dropped if offline; they are high-volume, low-stakes
        telemetry where an occasional gap is acceptable.
  \item \textbf{Voice-emotion audio segments --- not buffered.} The short audio clips
        for acoustic emotion are deliberately \emph{not} queued offline: raw audio is
        large, and we do not want it accumulating on the device (consistent with the
        raw-audio-deletion policy).
  \item \textbf{Heartbeats --- intentionally lost, and that is the signal.} Liveness
        pings are not queued; when the device is offline the pings simply stop, and the
        server's watchdog surfaces this to the parent as a ``monitoring has gone silent''
        alert (subject to quiet hours). An offline gap is therefore never invisible to
        the parent.
\end{itemize}
On the \textbf{Parent app} side, the dashboard, alerts and reports are read-only views
of the cloud database and require connectivity: with no network the parent sees the last
successfully fetched data (alert and dashboard polling simply retry on the next tick)
rather than stale-but-live-looking numbers, and any child data captured offline appears
as soon as the Child app's deferred uploads reach the server. Because delivery is
server-mediated, the parent view is always eventually consistent with what the Child app
managed to persist and upload. A natural extension identified for future work is an
on-device session buffer that records start/end times locally and back-fills them when
connectivity returns, closing the ``session started while offline'' gap above.
...
round trip, so a gaming session that begins fully offline is not recorded; if
connectivity returns mid-game, tracking begins from that moment (the offline head of the
session is lost, and detection retries every 5\,s until it succeeds). A session
\emph{end} that fails to send is healed server-side: the stale-session watchdog closes
the open
...
(isotonic), so reported confidences are trustworthy. The accuracy is on a synthetic
...
\subsection{Chat toxicity model (gaming-domain trained; honest, two views)}
...
$0.066 \rightarrow 0.064$), fused with the keyword lexicon.
\begin{center}
\begin{tabular}{lccc}
\toprule
\textbf{Evaluation set} & \textbf{Precision (toxic)} & \textbf{Recall (toxic)} & \textbf{Note} \\
\midrule
Balanced held-out & 0.896 & 0.953 & optimistic view; PR-AUC 0.938 \\
General realistic stream @0.50 & 0.053 & 0.953 & 0.7\% toxic base rate \\
General realistic stream @0.90 (alert) & 0.089 & 0.496 & out-of-domain view \\
\bottomrule
\end{tabular}
\end{center}
...
is $0.84$ in-domain at precision $0.62$), so a genuinely toxic session almost always
surfaces even when no single line crosses the per-message cutoff; requiring three
independent flags keeps the aggregate alert's false-positive rate low.
General-corpus behaviour remains reported above for honesty: out-of-domain precision
...
clips, 38 speakers, \emph{Urdu} --- included deliberately for its phonetic proximity
...
($31$ of $153$ speakers, $2{,}791$ rows; no test speaker appears in training):
\begin{center}
\begin{tabular}{lcccc}
\toprule
...
\midrule
Accuracy & \multicolumn{4}{c}{\textbf{0.57}} \quad (chance $= 0.25$) \\
Macro-F1 & \multicolumn{4}{c}{0.57} \\
\bottomrule
\end{tabular}
\end{center}
...
recognising people rather than emotions (\S\ref{sec:evalmethod}). $0.57$ on unseen
speakers across three languages is the honest generalisation number, and it is why
voice keeps the lowest, corroborating-only ensemble weight. The earlier measured
...
caveat, stated plainly: both corpora are \emph{acted, adult} speech --- closer to
...
each significant. Two honest nuances: the keyword fusion buys only $+0.010$ PR-AUC
...
plateaus (ties) that coarsen ranking. We accept that deliberately --- calibration is
what makes the $0.90$ threshold \emph{mean} ``$95\%$ precision'', and threshold
...
\begin{tabular}{lcc}
\toprule
\textbf{Feature set} & \textbf{Accuracy [95\% CI]} & \textbf{Macro-F1} \\
\midrule
Objective-10 (deployed)          & 0.916 [0.909, 0.923] & 0.918 \\
All-20 (incl.\ derived proxies)  & 0.919 [0.912, 0.926] & 0.921 \\
Volume features only (5)         & 0.871 [0.861, 0.879] & 0.874 \\
Pattern features only (5)        & 0.902 [0.894, 0.910] & 0.905 \\
\bottomrule
\end{tabular}
\end{center}
The derived proxies add nothing beyond the objective ten (overlapping CIs ---
confirming \S\ref{sec:features} empirically). More interesting: the five
\emph{pattern} features (late-night ratio, streak, binges, breaks, rapid re-logins)
alone recover $0.902$, while the five \emph{volume} features (hours, sessions,
duration) reach only $0.871$ --- the model leans on \emph{how} a child plays more
than on \emph{how much}, which is precisely the DSM-5/ICD-11 framing that motivated
the feature design.
...
and the measured (not asserted) consequence.
...
speech (RAVDESS+CREMA-D, 115 speakers): $0.572$ accuracy replaces the $0.9925$
synthetic figure (\S\ref{sec:voiceresults}); risk-vs-next-day-mood correlation
computable from data already collected \\
\addlinespace
Chat model retrained on gaming-domain data; alert threshold re-chosen ($0.95$/$0.97$);
session-level streak alert &
...
Behaviour: five families within $0.7$\,pp (even the linear baseline) $\rightarrow$
...
compared, RBF-SVM adopted, real-speech accuracy $0.572 \rightarrow 0.645$ \\
\addlinespace
...
because \emph{real} class overlap is larger --- the more honest number; direction
...
voice gains EMO-DB (German) + URDU, reaching $163$ speakers / 3 languages &
...
choice &
Leakage quantified ($0.657 \rightarrow 0.57$, and it had been selecting the wrong
...
\begin{itemize}[leftmargin=1.4em]
  ...
  \item A layered test program, all passing: a 56-test API suite (isolated database;
        ...
        on every push), a \textbf{81-check functional sweep run in production auth mode} with real
        ...
        Android Lint with zero errors, and 29 JVM unit tests (run in CI) pin the
        offline chat queue's delivery semantics, the WAV container encoding, IME
        keystroke-to-sentence reconstruction, and the parent-alert triage rules.
  \item Robustness under load on free-tier hosting: pooled, probe-validated connections
        ...
        landed --- p50 91\,ms, p95 686\,ms --- via the permanent
...
Reliability/anti-tamper & Heartbeat watchdog (uninstall/kill/offline alert, child-local quiet hours), optional Device Admin (uninstall block + instant disable alert), battery-optimisation exemption, double-start-safe services, consent-gated boot resume, parent-PIN lock on logout/settings (server-verified, constant-time), logout \emph{and} login alerts with honest monitoring status, idempotent session-end, self-scoring stale-session guard, per-child risk notifications, multi-guardian FCM push (lazy-init) \\
...
voice channels already trained on real data, the behaviour labels grounded in real
surveys, and the pilot instruments (threshold tuner, drift monitor, reflections
correlation) built and tested, that loop is the last credible step from a working
prototype to a validated product.
...
mitigations. What remains falls into two honest categories: work whose \emph{value}
...
\begin{itemize}[leftmargin=1.4em]
  \item \textbf{Feedback-driven retraining, stage two.} Threshold tuning from parent
        ...
        variables); what remains data-bound is learning the ensemble \emph{weights}
        and retraining the models on real labels --- the change that converts
        synthetic accuracy into validated performance. Needs a pilot at scale.
...
\begin{itemize}[leftmargin=1.4em]
  \item \textbf{Expert / clinical validation.} Validate the risk bands against an
        ...
        psychologist for the clinical reading. This is the honest path from a screening
