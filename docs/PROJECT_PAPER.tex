...
in-game and offensive-language corpora and reaches $0.95$ precision at its alert
threshold on held-out real gaming chat; the voice model is trained on $9{,}817$ real
...
into a single \emph{calibrated, SHAP-explainable} risk band; (iii)~a production-engineered,
privacy-conscious two-app + cloud system with tamper- and capture-permission health
monitoring and two-way nudges (guidance, not just alerts); and (iv)~an honest,
realistic-stream evaluation. The novelty lies in the deployed system and the canvas-game
