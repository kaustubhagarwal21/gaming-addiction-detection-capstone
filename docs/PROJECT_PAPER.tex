...
\begin{enumerate}[leftmargin=1.4em]
  ...
  \item Tamper resistance and resilient delivery: a self-restarting foreground service,
        a parent-PIN lock on logout/settings, a server-side heartbeat watchdog that alerts
        the parent when monitoring goes silent (uninstall / kill / offline), and real-time
        alerts pushed to \emph{every} guardian device in a family.
...
the system falls back to the acoustic label alone.
...
child swipes the app away from recents, and resumes on device boot. A child cannot
quietly disable it: logging out and changing settings require the family parent PIN,
verified server-side so the PIN is never exposed to the child, and a logout itself
raises a parent alert. Because Android gives an app no callback at its own uninstall,
...
offline --- re-arming once the app checks in again. Blocking uninstall outright (Android
Device Admin) was considered but deferred as too intrusive for a consent-based model.
...
Capture & Custom-keyboard (IME) + Accessibility typed-chat (canvas games included), on-device Vosk STT, voice features, real behavioural metrics \\
ML & Calibrated RF behaviour + SHAP, LogReg/TF-IDF chat, GBM voice, availability-weighted fusion, late-voice re-score \\
Credibility & Probability calibration, anomaly detection, observation mode, honest model card \\
Parent value & Per-day weighted risk + ``why'' (SHAP), 14-day trend, recently/most-played games, real-time alerts, weekly PDF, feedback loop, limits, send-nudge-to-child, switch child without re-login \\
Child wellbeing & Mira check-in, gamified daily check-in + streak, reflections, breathing, break/limit nudges \\
Reliability/anti-tamper & Heartbeat watchdog (uninstall/kill/offline alert), parent-PIN lock on logout/settings, logout alert, self-restart + boot resume, multi-guardian FCM push (lazy-init) \\
...
real-time multi-guardian alerts, and a passing test suite. Our central
...
\begin{itemize}[leftmargin=1.4em]
  ...
  \item \textbf{Hardened tamper-blocking (Device Admin).} The current heartbeat watchdog
        \emph{detects} uninstall or disabling after the fact; registering as an Android
        Device Administrator would \emph{block} casual uninstall and fire an instant alert
        the moment removal is attempted. It was deferred deliberately --- the
        ``device administrator'' permission is intrusive and changes the consent UX --- but
        is a clean, codebase-only addition for families that want it.
  \item \textbf{Smarter presence detection.} The silence watchdog cannot yet tell an
        uninstalled app from a phone that is merely powered off or offline (at school, or
        asleep). Restricting the alert to expected-active windows, or having the app flush
        a ``going offline'' marker on a clean shutdown, would cut false alarms.
\end{itemize}
