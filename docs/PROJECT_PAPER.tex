...
chat toxicity, and voice emotion --- into a single risk band (Casual / At-Risk /
Addicted; surfaced to families as Low / Some / High concern), with the behaviour
and chat probabilities isotonic-calibrated. The system comprises two native Android applications (a Child
...
of evaluation --- and every model choice is backed by a measured bake-off, the
feature and corpus choices by ablations with bootstrap confidence intervals, and
the fusion priors by sensitivity analyses on live pilot decisions. The pipeline is
...
speech clips across $154$ ingested speakers in three languages and evaluated
...
parent-feedback loop that converts real accurate/false-alarm verdicts into measured
threshold recommendations (weak labels, usable for tuning today and as one input to
future retraining) plus a drift monitor for the pilot phase --- and a consented live family pilot has since
...
score} for the calibrated channels --- behaviour and chat --- (how honest the served
probabilities are; the voice channel reports accuracy only, as its four-class output
...
\section{Results}
% =====================================================================
We report the honest, current metrics from the deployed models (one deliberate
...
the reported figures. The reported (lower) numbers are retained as the honest
published values; the assembly now de-duplicates for future retrains (majority
