...
(transcribed on-device; short WAV segments are also sent over HTTPS for acoustic
inference and deleted server-side after feature extraction); the voice model is trained on $9{,}817$ real
...
\end{itemize}
We note honestly that, unlike a fully on-device design, chat text and short raw-audio
...
\paragraph{A crisis path is a requirement, not a feature.} An external code audit of
the deployed build found that Mira's intent rules had no branch for self-harm: a child
...
(\S5.5) --- this detector is deliberately \emph{recall}-first: gaming hyperbole (``I'll
...
\midrule
Screen-time only (hours --- the baseline) & 0.702 [0.690, 0.714] & 0.709 \\
...
\end{tabular}
\end{center}
The first row is the \emph{thesis baseline}: the project's core claim is that
multimodal behaviour patterns beat screen-time counting, so the table must contain
screen-time counting. Deliberately given its best shot --- both hours features, the
same model family, protocol and CIs as every other row --- a well-fit hours-only
model reaches $0.702$, \textbf{21.4 points below the deployed feature set} with
cleanly disjoint intervals: the central claim is measured, not asserted. The derived
...
\begin{itemize}[leftmargin=1.4em]
  ...
  \item A layered test program, all passing: a 169-test API suite (isolated database;
...
\begin{itemize}[leftmargin=1.4em]
  ...
  \item \textbf{Tamper/permission monitoring caught two real events.} On one device
        ...
        participant's right.
  \item \textbf{The fusion-prior audit exposed both sensitivity and a unit-of-analysis
        limitation.} An initial exploratory run over 1{,}961 served snapshots suggested
        that weight perturbations changed at most $0.3\%$ of bands, while removing the
        genre multiplier changed $36\%$ and $\pm0.05$ threshold shifts changed up to
        $20\%$. A code audit then found that long sessions contributed many correlated
        live re-scores and floating-point boundary comparisons omitted 14 of the intended
        37 weight combinations. The corrected
        \texttt{ml/analyze\_fusion\_sensitivity.py} uses exactly the latest prediction
        from each completed session, preserves the observation-mode ceiling, covers all
        37 grid points, and refuses output if it cannot reproduce the served score and
        category. The snapshot figures are therefore retained as exploratory evidence,
        not a confirmatory claim; the production pilot database must be rerun through the
        corrected analysis. A prevalence-anchoring method for
...
voice-mitigation analysis (\S6.3), with the frustrated-label share steady at 79\%.
...
observable pattern}, not as noise. Second, an honest null: the child-side reflection
