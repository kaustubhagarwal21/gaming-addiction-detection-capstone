...
adult IGDS9-SF survey ($n{=}87$), the score correlates with the clinical instrument at
...
factors~\cite{ergin2025}; none deploys a passive, multi-signal screen to a guardian.

We built and deployed such a system and, distinctively, evaluated it the way a
screening instrument should be: real data where it exists, honest numbers where it
...
\item An evaluation discipline with teeth: speaker-independent splits, one-component
...
and the severity base rate ($6.4\%$ scoring $\geq$$36$) taken from an $11{,}191$-player
IGDS9-SF dataset~\cite{igdslatam2024}. We state throughout that the resulting accuracy
...
$0.25$). Random splits reported $0.657$ --- a 9-point speaker-leakage inflation that
also flipped the bake-off winner. Frozen wav2vec~2.0 embeddings with the same
classifier reach $0.776$ on the identical split, quantifying the headroom a deployable
distillation could recover.
...
instrument; it is a fully built, honestly measured screening pipeline in which every
