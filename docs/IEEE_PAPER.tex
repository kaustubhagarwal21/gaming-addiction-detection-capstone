...
instrument at Spearman $\rho{=}0.317$ ($95\%$ CI $[0.137,0.478]$), leads the
self-reported screen-time baseline in $97.4\%$ of paired resamples
($\Delta\rho{=}{+}0.167$), survives partialling that baseline out ($\rho{=}0.303$,
CI excluding zero), and the signal is carried by \emph{pattern} features (composite
$\rho{=}0.330$) rather than \emph{volume} ($0.128$, CI including zero) --- the design premise confirmed rather than
...
value is what we report.) IGDS totals averaged $17.6$ (SD $6.0$; range $9$--$37$);
one respondent was in the disordered range. The model was fitted before the survey
...
tracks the instrument at $\rho{=}0.317$, CI excluding zero. The screen-time baseline
every commercial tool ships reaches $0.147$ with a CI spanning zero; a paired
bootstrap puts the model ahead in $97.4\%$ of resamples ($\Delta\rho{=}{+}0.167$),
though the difference's interval now grazes zero ($[-0.002,+0.341]$; it excluded
zero pre-batch), so the incremental claim rests on the partial correlation:
partialling hours out barely moves the score's correlation ($0.303$, CI excluding
zero) --- it is not screen time in another unit. All five pattern features out-rank
all five volume features; the composites diverge ($0.330$ vs $0.128$, the latter's
CI including zero). Self-reported toxic-chat involvement tracks severity at
...
telemetry-linked cohort (\S V). On magnitude: $\rho{=}0.317$ ($\approx$$10\%$ shared
