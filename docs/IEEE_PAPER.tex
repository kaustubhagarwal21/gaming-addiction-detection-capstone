% IEEE conference-format distillation of docs/PROJECT_PAPER.tex (the 48-page report).
...
\title{Measuring \emph{How} People Play, Not Just How Long:\\
A Multimodal, Privacy-Conscious Gaming-Risk Monitor for Guardians, with Construct-Validity Evidence}

% Flip \anontrue for double-blind venues: strips names, affiliation and the repo URL.
\newif\ifanon \anonfalse
\ifanon
\author{\IEEEauthorblockN{Anonymous submission}\IEEEauthorblockA{Code and data artefacts will be released on acceptance.}}
\else
\author{
...
a deployed, three-tier gaming-risk monitor --- a passive-capture Android child app, a
cloud ML backend, and a parent app --- that fuses three channels: ten \emph{measured}
...
voice $0.574$ speaker-independent, exposing an 8.3-point leakage inflation in random splits).
Critically, we then validate the served risk score \emph{externally}: in an anonymous
adult IGDS9-SF survey ($134$ raw; $104$ usable, $103$ scoreable), the score correlates with the clinical
instrument at Spearman $\rho{=}0.317$ ($95\%$ CI $[0.137,0.478]$; the
pre-designated primary endpoint), leads the self-reported screen-time baseline in
$97.4\%$ of paired resamples ($\Delta\rho{=}{+}0.167$, $95\%$ CI $[-0.002,+0.341]$
--- suggestive, not established two-sided), survives partialling that baseline out
($\rho{=}0.303$, CI excluding zero), and the signal is carried by \emph{pattern}
features over \emph{volume} features by a formally tested margin (paired composite
difference $+0.199$, $95\%$ CI $[+0.019,+0.380]$) --- the design premise confirmed rather than
...
counters measure volume, which objective-telemetry studies find to be a weak
correlate of well-being outcomes~\cite{johannes2021,vuorre2022}. Section~II situates
this work; in brief, no prior system combines passive multi-signal capture, a
guardian-facing risk channel, and an external validity check against a clinical IGD
instrument.
...
\item External construct validation of the served score against IGDS9-SF~\cite{igds9sf}
  in $104$ usable respondents, with incremental validity over screen time, a
...
Passive smartphone sensing of behaviour and mental state has a decade of precedent:
StudentLife inferred stress and academic outcomes from phone telemetry in a student
cohort~\cite{studentlife2014}, and Shin and Dey detected \emph{problematic} smartphone
use from passive usage features validated against a self-report scale~\cite{shin2013},
with qualitative grounding in Lee et al.'s study of smartphone overuse~\cite{lee2014}.
Our behavioural channel follows this tradition but targets gaming-specific pattern
features and serves a guardian, not the user. On the outcome side, large
objective-telemetry studies find raw play \emph{time} only weakly related to
well-being~\cite{johannes2021,vuorre2022} --- the empirical basis for our
pattern-over-volume design premise. IGD-specific computational work has modelled
survey data~\cite{huang2024,strojny2024} or reviewed family factors~\cite{ergin2025}
rather than deploying capture. Commercial guardian tools (screen-time dashboards;
content monitors such as Bark) do deploy multi-signal capture, but report volume or
keyword hits rather than a validated severity construct, and the HCI literature
documents both their surveillance risks and children's own views of
them~\cite{ghosh2018} --- concerns our consent, transparency and
visibility-over-prevention design responds to (\S\ref{sec:ethics}). The specific gap
this paper addresses is the conjunction: passive multimodal capture, a
guardian-facing calibrated risk signal for IGD specifically, and an external
construct-validity check of the served score against a clinical
instrument~\cite{igds9sf}.
...
$0.511$; removing char\_wb n-grams costs $0.030$; removing keyword fusion costs recall
at the alert threshold $0.363\!\rightarrow\!0.278$ (MCC $0.541\!\rightarrow\!0.472$)
under the ablation protocol.

\textbf{Voice.} Speaker-independent accuracy $0.574$, macro-F1 $0.568$ (chance
$0.25$); random splits had reported $0.657$, an 8.3-point speaker-leakage inflation we
...
per row, each row \emph{retrained from scratch} on its own fixed evaluation protocol
--- voice rows use the ablation's un-augmented speaker-independent test set (1{,}880
clips), so the full-recipe row ($0.562$) differs from the deployed training run's
held-out figure ($0.574$); chat rows likewise differ slightly from the deployed
artefact's serving metrics.}
\label{tab:abl}
\begin{tabular}{@{}llc@{}}
\toprule
...
derivation, fitted scaler, calibrated forest, identical risk formula). The main
wave (7--9 Aug) stood at $112$ raw responses; the form remained open and a passive
late batch of $22$ arrived 13--20 Aug. Per a rule fixed before any late data
existed, all analyses use the final $134$ raw responses: $104$ usable (20 failed
eligibility, 10 an attention check), $103$ scoreable. (The pre-batch snapshot read
$\rho{=}0.352$ $[0.158,0.521]$; the late batch attenuated it, and the attenuated
value is what we report.) The cohort is young ($79.6\%$ aged 18--24) and non-clinical: IGDS totals averaged
$17.6$ (SD $6.0$; range $9$--$37$); one respondent was in the disordered range. The model was fitted before the survey
existed and nothing was tuned on it.

\begin{table}[t]
\centering\footnotesize
\caption{Construct validity against IGDS9-SF ($n{=}103$; Spearman, bootstrap 95\% CIs).}
\label{tab:survey}
\begin{tabular}{@{}lcc@{}}
\toprule
Predictor & $\rho$ & 95\% CI \\
\midrule
\textbf{Served risk score} & $\mathbf{0.317}$ & $[0.137, 0.478]$ \\
Self-reported hours/week (baseline) & $0.147$ & $[-0.053, 0.339]$ \\
Paired difference (model $-$ hours) & $+0.167$ & $[-0.002, +0.341]$ \\
\textbf{Partial (hours removed)} & $\mathbf{0.303}$ & $[0.110, 0.476]$ \\
\midrule
Pattern-feature composite & $0.330$ & $[0.154, 0.489]$ \\
Volume-feature composite & $0.128$ & $[-0.083, 0.328]$ \\
Toxic-chat involvement (chat premise) & $0.323$ & $[0.144, 0.491]$ \\
Excluding 9 straight-liners ($n{=}94$) & $0.292$ & $[0.104, 0.461]$ \\
\bottomrule
\end{tabular}
\end{table}
...
\caption{Per-feature $|\rho|$ against IGDS9-SF total ($n{=}103$; per-feature
SEs ${\approx}0.10$, so individual rankings are descriptive). The formal test is the
paired composite contrast: pattern $-$ volume $= +0.199$, $95\%$ CI $[+0.019,+0.380]$.}
\label{fig:feat}
\end{figure}

\textbf{Results} (Table~\ref{tab:survey}, Fig.~\ref{fig:feat}). The
construct-validity correlation was the pre-designated primary endpoint (stated in
the analysis script's documentation before data collection); the remaining results
are secondary. The served score tracks the instrument at $\rho{=}0.317$, CI
excluding zero. The screen-time baseline every commercial tool ships reaches
$0.147$ with a CI spanning zero; a paired bootstrap puts the model ahead in
$97.4\%$ of resamples ($\Delta\rho{=}{+}0.167$, one-sided $p{\approx}0.026$), but
the two-sided interval grazes zero ($[-0.002,+0.341]$; it excluded zero pre-batch),
so superiority over the baseline is suggestive, not established. The partial
correlation establishes \emph{non-redundancy}: partialling hours out barely moves
the score's correlation ($0.303$, CI excluding zero) --- the score is not screen
time in another unit, though with a single-item, noisily measured covariate this
under-controls for true screen time~\cite{westfall2016} and cannot itself
establish superiority. The pattern-over-volume premise \emph{is} formally
supported: a paired bootstrap on the composite difference gives
$+0.199$ ($95\%$ CI $[+0.019,+0.380]$), and descriptively all five pattern
features out-rank all five volume features ($0.330$ vs $0.128$ composites). Self-reported toxic-chat involvement tracks severity at
$0.323$, replicating in this population the $r{=}{+}0.156$ premise borrowed
from~\cite{igdslatam2024}.
\textbf{Two results against the system.} The genre multiplier is unsupported
(Kruskal--Wallis across seven genres, $H{=}4.59$, $p{=}0.598$ --- weakening at every
growth of the sample, from $p{=}0.159$ at $n{=}80$ --- and the group ordering no
longer matches the multiplier's assumption; power $32\%$, needing
$n\!\approx\!400$), so it is retained-and-flagged rather than defended --- removing
it flips $34\%$ of served bands. Four of five parent-facing proxies fail to track
the IGDS item they are named after (only ``craving'' holds, $\rho{=}0.322$; the
rest read $+0.020$ to $+0.092$); we renamed the served labels to
behaviour-descriptive wording. \textbf{Not established:} sensitivity /
...
longitudinal claim. A grid-searched threshold refit ($T_1{=}0.51,T_2{=}0.83$,
$\kappa{=}0.177$) was measured and \emph{declined}: the fitted $T_2$ moved by
$0.12$ on one late batch --- the demonstration that it encodes the sample's missing
severity tail, not a clinical boundary.
...
concern: the incremental effect over self-reported hours ($\Delta\rho{=}{+}0.167$,
leading in $97.4\%$ of paired resamples) is a \emph{within-method} contrast, since
hours are self-reported on the same form
and share any method inflation --- though the score aggregates seven items while
the baseline is a single item, so part of its advantage may be differential
reliability rather than construct coverage~\cite{westfall2016}; the
pattern-vs-volume separation is a \emph{between-feature} contrast that shared
method cannot manufacture; and the
...
telemetry-linked cohort (\S\ref{sec:ethics}). On magnitude: $\rho{=}0.317$ ($\approx$$10\%$ shared
variance) is modest in absolute terms, with a wide interval at $n{=}103$; it sits
...
291 automated tests run in CI (181 backend on SQLite \emph{and} Postgres~16; 110
...
was notified in writing (official institutional e-mail of 14~August~2026, copied to
the other authors, disclosing the design and requesting direction should additional
approval be required), sent while the form was open --- after the main wave, before
close-out and before analysis of the final sample. That is documented faculty
...
This work was carried out as PES University capstone project PW26\_SAS-03 under the
guidance of Prof.\ Shridevi Sawant.
\fi
