...
check. 55 backend tests — run in CI against both SQLite and Postgres 16, the
production dialect — cover the authz matrix, including regression tests for the
alert-ownership gap we found and fixed; 22 Android JVM unit tests guard the
offline-queue, WAV, and alert-triage logic.
